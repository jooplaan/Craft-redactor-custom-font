(function ($R) {
    ...
    $R.add('plugin', 'customFont', {
        init: function (app) {
            this.app = app;
            this.toolbar = app.toolbar;
        },
        start: function () {
            this.button = this.toolbar.addButton('customFont', {
                title: Craft.t('redactor-custom-font', 'Custom font icons'),
                api: 'plugin.customFont.open',
                icon: '<i class="fa fa-smile-o"></i>'
            });
        },
        modals: {
            'customFontModal': '<section id="redactor-modal-advanced">'
                + '<div class="iconFilterInput"><input placeholder="' + Craft.t('redactor-custom-font', 'Search for icons') + '" id="iconSearchBox" type="text" /></div>'
            ...
            customFontModal: {
                open: function (modal, form) {
                    ...
                    for (var z = 0; z < lencount; z++) {
                        ...
                        var listcons = $('<i class="fa fa-' + icon + '" id="redactor-fa-' + icon + '"><span>' + icon + '</span></i>');
                        blkstr.push('<div><i class="fa fa-' + icons[z] + '" id="redactor-fa-' + icons[z] + '"><span>' + icons[z] + '</span></i></div>');
                ...
                insert: function (modal, form) {
                    var str = $(".iconContents div.active i span").text();
                    //console.log(str);

                    this.app.api('module.modal.close');
                    this.app.selection.restore();

                    var node = $('<span />').html('<i class="fa fa-' + str + '"></i>', false);
